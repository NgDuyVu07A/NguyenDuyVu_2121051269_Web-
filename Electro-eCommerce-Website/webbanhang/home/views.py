...
# 1. TRANG CHỦ
def index(request):
    # Dùng icontains và từ khóa ngắn để "né" hoàn toàn lỗi khoảng trắng hay Unicode tiếng Việt
    ...
    if search_query:
        # Nếu có từ khóa, áp dụng lọc theo tên cho cả 2 danh sách
        phones = phones.filter(name__icontains=search_query)
        tablets = tablets.filter(name__icontains=search_query)

    # Gửi 2 biến phones và tablets sang giao diện HTML
    context = {
        'phones': phones,
        'tablets': tablets
    }
